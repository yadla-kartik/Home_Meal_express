...
import { sendUserOtp, verifyUserOtp } from '../../../../services/userAuthService'

const OTP_LENGTH = 6
const RESEND_SECONDS = 30
const SESSION_KEY = 'userOtpPayload'
...
function Otp({ onVerify }) {
  ...
  useEffect(() => {
    ...
    const fallbackPath = location.pathname.startsWith('/chef') ? '/chef/login' : '/login'
  ...
  const handleVerify = async () => {
    ...
    try {
      const res = await verifyUserOtp({ ...pending, otp: finalOtp })
      if (!res?.success) {
  ...
  const handleResend = async () => {
    ...
    try {
      const res = await sendUserOtp({ ...pending, __mode: 'resend' })
      if (!res?.success) {
